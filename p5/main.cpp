...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("CAVE RUN!", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
...
void Render() {
    ...
    if (currentScene->state.player->gameOver) {
        Util::DrawText(&program, fontTextureID, "YOU LOSE", 1, -0.5f, glm::vec3(currentScene->state.player->position.x, -2, 0));
    }
    if (currentScene->state.player->reached) {
